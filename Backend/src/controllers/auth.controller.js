...
export const postLogin = async (req, res) => {
	...
	try {
		// Si el identifier parece un email, lo dejamos igual, si no, lo pasamos a minúsculas
		const isEmail = identifier.includes("@");
		const identifierToFind = isEmail ? identifier : identifier.toLowerCase();

		// Busco el usuario por email o username (username siempre en minúsculas)
		const user = await repoFactory.findUser({
			$or: [{ email: identifierToFind }, { username: identifierToFind }],
		});
...
// 		await repoFactory.saveUser(usernameLower, email, password);
...
export const postSignup = async (req, res) => {
	...
	// Verifico si el username ya existe (siempre en minúsculas)
	const usernameCount = await repoFactory.countUsers({ username: usernameLower });
	if (usernameCount > 0) {
		return res.status(400).json({
			message: "El nombre de usuario ya está registrado",
		});
