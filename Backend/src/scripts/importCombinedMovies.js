...
import {
	fetchDiscoverMoviesMultiLang,
	fetchMostKnownMovies,
	fetchMovieDetails,
	fetchMovieCredits,
...
const TOTAL_DESEADO = 120;

const importCombinedMovies = async () => {
	let repository;

	try {
		repository = await createRepositoryAdapter("mongoose");

		// Traer películas de ambos métodos
		const discoverMovies = await fetchDiscoverMoviesMultiLang(1);
		let page = 1;
		let mostKnownMovies = [];
		while (mostKnownMovies.length < TOTAL_DESEADO) {
			const pageMovies = await fetchMostKnownMovies(page);
			if (!pageMovies.length) break;
			mostKnownMovies = mostKnownMovies.concat(pageMovies);
			page++;
		}

		// Combinar y eliminar duplicados por tmdbId
		const allMovies = [...discoverMovies, ...mostKnownMovies];
		...
		// Mezclar el array para que los resultados queden entreverados
